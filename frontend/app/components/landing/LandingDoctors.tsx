  ...
  return (
    <section className="section section-tint" id="doctors-section">
      ...
      {docs.isLoadingDoctors ? (
        <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b', fontSize: '0.9rem', fontWeight: 600 }}>Loading doctors...</div>
      ) : docs.doctorsList.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b', fontSize: '0.9rem', fontWeight: 600 }}>No active doctors registered.</div>
      ) : (
              ...
              <div key={doc.id} className="doctor-accordion-item">
                <div className="doctor-accordion-header" onClick={() => docs.toggleDoctorAccordion(doc.id)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', border: '2px solid #0284c7', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#cff4fc', color: '#087990', fontSize: '0.85rem', fontWeight: 800, flexShrink: 0 }}>
                      {doc.avatarInitials}
                    </div>
                    <div>
                ...
                {isExpanded && (
                  <div className="doctor-accordion-content">
                    ...
                    <p style={{ fontSize: '0.82rem', color: '#475569', marginBottom: '0.75rem' }}><strong>Timeline & Hours:</strong> {doc.hours} (30-min slots)</p>
