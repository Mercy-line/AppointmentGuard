...
const DEFAULT_DOCTORS: Doctor[] = [
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userFromAPI) {
        setCurrentUser((userFromAPI as any).user || userFromAPI);
    ...
    // 2. Check Seeded Users Fallback
    const seeded = SEEDED_USERS[emailTrimmed];
    if (seeded && passwordTrimmed === 'PatientPass123!') {
  ...
  // User Registration Submit Handler (Admin create user)
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resData = await registerAPI({
        ...
        name: regName.trim(),
        role: regRole,
        spec: regRole === 'DOCTOR' ? regSpecialization.trim() : undefined
      });

      const newUser = (resData as any).user || resData;
      ...
      if (newUser) {
        ...
        if (regRole === 'PATIENT') {
          ...
          if (updatedPatients && Array.isArray(updatedPatients)) {
            ...
            setPatientsList(prev => [{ id: newUser.id, name: newUser.name, email: newUser.email, username: newUser.username || newUser.email.split('@')[0], date_joined: 'Just now' }, ...prev]);
          }
        }
